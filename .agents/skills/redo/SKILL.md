...
argument-hint: <thread|blog|newsletter> <folder> [optional notes in quotes]
---
...
The user's request will look like one of:
...
Extract:
- **writer** — must be one of: `thread`, `blog`, `newsletter`. If not, tell the user the valid options and stop.
...
Map `writer` → subagent name: `thread` → `thread-writer`, `blog` → `blog-writer`, `newsletter` → `newsletter-writer`.
